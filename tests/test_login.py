...
def test_invalid_user_login(page):
    ...
    login_page = LoginPage(page) # page, hence object is created

    home_page.click_my_account()
    home_page.click_login()

    login_page.set_email(Config.invalid_email) # read data from config.py file, only data hence no object is created
    login_page.set_password(Config.invalid_password) # read data from config.py file
